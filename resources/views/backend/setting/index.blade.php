    ...
    <script type="text/javascript">
        $(function() {
            var table = $('.data-table').DataTable({
                processing: true,
                serverSide: false,
                ajax: route('websettingContacts.index'),
                columns: [
                {
                    data: 'data',
                    name: 'data'
        ...
        $(document).on('click', '.status', function() {
            ...
            $.ajax({
                type: 'POST',
                url: route('update.websetting.status'),
                data: {
                    id: id,
                    status: status
                },
                success: function(data) {
                    ...
                    if (status == 'active') {
                        $('.status#switch' + id).attr('value', 'inactive');
                    } else {
                        $('.status#switch' + id).attr('value', 'active');
                    }
        ...
        function trashRecord(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: route('websettingContacts.destroy', id),
                        type: 'DELETE',
                        beforeSend: function() {
                            $.LoadingOverlay("show");
                        },
